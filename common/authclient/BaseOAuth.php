...
abstract class BaseOAuth extends BaseClient
{
    ...
    protected function restoreAccessToken()
    {
       yii::error('ingresando a la funcion',__FUNCTION__);
        ...
        yii::error('Parametros token',__FUNCTION__);
        yii::error($token->params,__FUNCTION__);
        yii::error('TOKEN',__FUNCTION__);
        yii::error($token,__FUNCTION__);
        $expirationDuration = $token->getExpireDuration();
    $saldo=time()-($token->createTimestamp+$expirationDuration);    

        //return (time() >= ($this->createTimestamp + $expirationDuration));
  yii::error('El toeken exporara en  '.$saldo.' Segundos',__FUNCTION__);
  
        //VAR_DUMP($token);DIE();
        //VAR_DUMP($token->params);DIE();
        if (is_object($token)) {
            yii::error('TOKEN es obejeto ',__FUNCTION__);
            /* @var $token OAuthToken */
             yii::error($token->getIsExpired(),__FUNCTION__);
             yii::error($this->autoRefreshAccessToken,__FUNCTION__);
            if ($token->getIsExpired() && $this->autoRefreshAccessToken) {
                 yii::error('refreshTOKEN de client zoom',__FUNCTION__);
