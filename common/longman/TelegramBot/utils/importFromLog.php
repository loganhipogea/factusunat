...
try {
    ...
    $telegram = new \common\longman\TelegramBot\Telegram($API_KEY, $BOT_NAME);
    ...
    foreach (new SplFileObject($filename) as $current_line) {
        ...
        if (!is_null($json_decoded)) {
            ...
            $telegram->processUpdate(new common\Longman\TelegramBot\Entities\Update($json_decoded, $BOT_NAME));
        }
        $update .= $current_line;
    }

} catch (common\longman\TelegramBot\Exception\TelegramException $e) {
    // log telegram errors
    echo $e;
}
