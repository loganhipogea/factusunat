               ...
               'buttons' => [  
			    ...
			    $url= Url::to(['masters/clipro/edita-centro','id'=>$model->codcen,'gridName'=>$zonaAjax,'idModal'=>'buscarvalor']);
                        ...
                        'delete' => function ($url,$model)use($zonaAjax) {                             
                              ...
                              return \yii\helpers\Html::a('<span class="btn btn-danger glyphicon glyphicon-trash"></span>', '#', ['rel'=>$url,/*'id'=>$model->codparam,*/'family'=>'holas','id'=>\yii\helpers\Json::encode(['id'=>$model->codcen,'modelito'=> str_replace('@','\\',get_class($model))]),/*'title' => 'Borrar'*/]);
                             }
