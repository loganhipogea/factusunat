<?php

namespace frontend\modules\op\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use frontend\modules\op\models\OpProcesos;

/**
 * OpProcesosSearch represents the model behind the search form of `frontend\modules\op\models\OpProcesos`.
 */
class OpProcesosSearch extends OpProcesos
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id'], 'integer'],
            [['numero', 'fechaprog', 'fechaini', 'numoc', 'codpro', 'descripcion', 'tipo', 'codestado', 'textocomercial', 'textointerno', 'textotecnico'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = OpProcesos::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
        ]);

        $query->andFilterWhere(['like', 'numero', $this->numero])
            ->andFilterWhere(['like', 'fechaprog', $this->fechaprog])
            ->andFilterWhere(['like', 'fechaini', $this->fechaini])
            ->andFilterWhere(['like', 'numoc', $this->numoc])
            ->andFilterWhere(['like', 'codpro', $this->codpro])
            ->andFilterWhere(['like', 'descripcion', $this->descripcion])
            ->andFilterWhere(['like', 'tipo', $this->tipo])
            ->andFilterWhere(['like', 'codestado', $this->codestado])
            ->andFilterWhere(['like', 'textocomercial', $this->textocomercial])
            ->andFilterWhere(['like', 'textointerno', $this->textointerno])
            ->andFilterWhere(['like', 'textotecnico', $this->textotecnico]);

        return $dataProvider;
    }
}
